...
import com.ggj.webmagic.autoconfiguration.TieBaSinglePageImageIdMessageListener;
...
import com.ggj.webmagic.tieba.service.TieBaPageInfoService;
import com.ggj.webmagic.util.CommonUtil;
...
public class WebmagicService {
	...
	@Autowired
	private TieBaPageInfoService tieBaPageInfoService;
	@Autowired
		...
		redisTemplate.executePipelined(new RedisCallback<Object>() {
			...
			public Object doInRedis(RedisConnection redisConnection) throws DataAccessException {
				...
				for (Iterator<String> iter = map.keySet().iterator(); iter.hasNext();) {
					...
					redisConnection.zAdd(key, Double.parseDouble(topBean.getIndex()),
							getByte(JSONObject.toJSON(topBean).toString()));
				}
				return null;
			}
		});
	...
	public void syncTieBaInfo() {
		String redisKey = TieBaSinglePageImageIdMessageListener.TIEBA_CONTENT_SINGLE_PAGE_IMAGE_INDEX;
		Set<byte[]> val = redisTemplate.execute(new RedisCallback<Set<byte[]>>() {
			public Set<byte[]> doInRedis(RedisConnection redisConnection) throws DataAccessException {
				byte[] key = getByte(redisKey);
				return redisConnection.sMembers(key);
			}
		});
		List<String> vals = Lists.newArrayList();
		for (byte[] bytes : val) {
			vals.add(getString(bytes));
		}
		tieBaPageInfoService.saveTiebaPageInfo(vals);
	}
	...
	/**
	 * 根据帖子最后最新更新日期显示图片 zrange 从小到大 zRevRange从大到小 优化前速度 2017-03-09 19:17:48.053
	 * INFO 30624 --- [p-nio-81-exec-2] com.ggj.webmagic.WebmagicService :
	 * zRevRange:201条数据 耗时：30s 2017-03-09 19:17:53.877 INFO 30624 ---
	 * [p-nio-81-exec-2] com.ggj.webmagic.WebmagicService : 循环:201条数据 耗时：5823s
	 * 优化后的速度 2017-03-09 19:45:26.124 INFO 31864 --- [p-nio-81-exec-2]
	 * com.ggj.webmagic.WebmagicService : zRevRange:201条数据 耗时：90ms 2017-03-09
	 * 19:45:26.474 INFO 31864 --- [p-nio-81-exec-2]
	 * com.ggj.webmagic.WebmagicService : 循环:201条数据 耗时：349ms redis批量数据用Pipeline
	 * 
	 * @param model
	 * @param tieBaName
	 * @param begin
	...
	public Map<String, List<String>> getTieBaImage(Model model, String tieBaName, Integer begin, Integer end) {
		...
		Set<String> sortPageIds = redisTemplate.opsForZSet()
				.reverseRange(TieBaImageIdMessageListener.TIEBA_CONTENT_UPDATE_TIME_KEY + tieBaName, begin, end);
		// log.info("redis zRevRange:{}条数据 耗时：{}ms", sortPageIds.size(),
		...
		int i = 0;
		for (String sortPageId : sortPageIds) {
			map.put(tieBaConfiguration.getTiebaContentPageUrl() + sortPageId,
					JSONObject.parseObject(listObject.get(i).toString(), ArrayList.class));
			i++;
		}
		// log.info("redis pipelined循环get:{}条数据 耗时：{}ms", sortPageIds.size(),
		// (System.currentTimeMillis() - beginTime));
		return map;
	}
	...
	public List<ContentBean> search(Model model, String keyWord, Integer from) {
		...
		SearchResponse response = elasticSearch.getTransportClient().prepareSearch(ElasticSearch.INDEX_NAME)
				.setTypes(ElasticSearch.TIEABA_CONTENT_TYPE).setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
	...
	public void uploadLocalDoc() {
		String autherName = "妲己不是妖";
		Map<String, byte[]> file = CommonUtil.getFileImg(autherName);
		ContentBean cb = new ContentBean();
		cb.setAuthorName(autherName);
		cb.setId("000000002");
		for (String key : file.keySet()) {
			convertImageUrl(key, file.get(key), cb);
		}

	}

	private String convertImageUrl(String key, byte[] image, ContentBean cb) {
		try {
			String qiniuUrlTml = cb.getAuthorName() + "|" + cb.getId() + "|" + key + ".jpg";
			String retQNUrl = qiNiuUtil.upload(qiniuUrlTml, image);
			TieBaImage tieBaImage = new TieBaImage();
			tieBaImage.setId(key);
			//imageUrl.replace(tieBaConfiguration.getTiebaWallImageUrl(), "").split("/")[1].split("\\.")[0]);
